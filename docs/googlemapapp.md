...
- **下载商家图片**：是否下载商家门店主图；不勾选可加快采集速度
- **代理地址**：如需通过代理访问 Google 地图，填写代理地址（如 `http://192.168.0.60:7893`）；本机可直接访问则留空
- **超时时间**：单页加载超时时间（秒）
...
- **操作按钮**：
  ...
  - **清空关键词**：清除已输入的关键词列表
  - **清空数据**：删除采集进度或数据结果（下拉菜单可选择清空范围）
  - **设置**：跳转回参数配置界面
- **统计信息**：实时显示总关键词数、待查询数、数据总量
- **数据列表 / 运行日志**：Tab 切换查看采集结果或运行日志
- **导出 Excel**：将采集到的商家数据导出为 Excel 文件

点击"开始采集"按钮后，程序将自动打开指纹浏览器跳转到 Google 地图首页，并等待用户进行登录（如需）。登录完成后，程序将自动开始按关键词组合搜索。采集过程中可随时点击"停止采集"暂停；再次点击"开始采集"将从断点续跑（已采集的组合会被自动跳过）。
...
> 如需对采集结果做 AI 精挑（文本评分 + 图片过滤 + 过滤导出），可在精挑流程中进一步筛选，最终只导出高价值线索。


## 清空数据

采集过程中，程序将记录关键词的工作结果，再次工作时成功或失败的关键词将被跳过。如需重新采集，可点击操作区的 **"清空数据"** 按钮，在下拉菜单中选择相应选项：

- **清空失败采集进度**：只删除所有失败的采集进度，保留成功采集的进度及数据结果
- **清空采集进度**：清空所有的采集进度，保留成功采集的数据结果
